...
from .expressions import mesh_potential, point_potential, polygon_potential
from .utils import area_centroid, construct_derivative, expand_tensor
...
class Electrode:
    ...
    def __init__(self, name="", dc=0.0, rf=0.0):
        self.name = name
        self.dc = dc
        self.rf = rf

    def potential(self, x, derivative=0, potential=1.0, out=None):
...
class CoverElectrode(Electrode):
    ...
    * The other electrodes in the `System` all need to have their
    ...
    def __init__(self, height=50.0, **kwargs):
...
class SurfaceElectrode(Electrode):
    ...
    def __init__(self, cover_height=50.0, cover_nmax=0, **kwargs):
...
class PolygonElectrode(SurfaceElectrode):
    """A polygonal patch electrode.

    Parameters
    ----------
    path : array_like, shape (n, 2)
        Polygon boundary as list of points. The polygon with positive
        orientation (counter clock wise) contributes with poisitive sign.
        The one with negative orientation contributes with negative sign.

    See Also
    --------
    Electrode
        `name`, `dc`, `rf` attributes/parameter
    """

    def __init__(self, path, **kwargs):
        super().__init__(**kwargs)
        self.path = np.asanyarray(path, np.double)
...
class PolygonPixelElectrode(SurfaceElectrode):
    ...
    def plot(self, ax, label=None, color=None, **kw):
        ...
        codes = np.concatenate(
            [
                np.r_[
                    ...
                    np.ones(len(p)) * mpl.path.Path.LINETO,
        ...
        patch = mpl.patches.PathPatch(
            path, facecolor=color, edgecolor=kw.pop("edgecolor", "none"), **kw
        )
        ax.add_patch(patch)
        if label is None:
            label = self.name
...
class MeshPixelElectrode(SurfaceElectrode):
    ...
    def __init__(self, points=[], edges=[], polygons=[], potentials=[], **kwargs):
...
class GridElectrode(Electrode):
    ...
    def __init__(self, data=[], origin=(0, 0, 0), spacing=(1, 1, 1), **kwargs):
    ...
    def potential(self, x, derivative=0, potential=1.0, out=None):
        ...
        for i in range(2 * derivative + 1):
            out[:, i] += potential * map_coordinates(dat[..., i], x.T, order=1, mode="nearest")
        return out
